...
#include <curses.h> // [1-5]コンソール入出力ヘッダーをインクルードする <conio.h>なんてないので<curses.h>を代わりに使う。けど結局<stdio.h>のgetchar関数にしている。
...
CHARACTER monsters[MONSTER_MAX] =
{
    ...
    // [5-1-8]MONSTER_SLIME     スライム
    {
        ...
        "スライム",     // [5-1-14]char name[6 * 2 + 1]    名前
...
// [5-3]コマンドの名前を宣言する
char commandNames[COMMAND_MAX][6 * 2 + 1] = {
...
void DrawBattleScreen(){
    // [6-2-1]画面をクリアする
    system("sh clear.sh");  // ここちょっと本家と違うよ
    // [6-2-2]プレイヤーの名前を表示する
...
void Battle(int _monster){
    ...
    // [6-4-6]キーボード入力を待つ
    getchar(); // <curses.h>のgetch関数はなんか使えないので<stdio.h>のgetchar関数で代用
        ...
        for (int i = 0; i < CHARACTER_MAX; i++){
            ...
            switch (characters[i].command){
                ...
                case COMMAND_FIGHT: // [6-4-12]戦う
                    ...
                    damage = 1 + rand() % characters[i].attack;
                    // [6-4-16]敵にダメージを与える
